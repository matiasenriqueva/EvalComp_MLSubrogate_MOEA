...
def rank_average_friedman_test(data: pd.DataFrame, output_dir="friedman_rank", indicator_name=""):
    latex_output = ""
    if type(data) == pd.DataFrame:
        data_values = data.values
        data_columns = data.columns
    data_rank = ranks(data_values)
    avg_ranks = np.mean(data_rank, axis=0)
    frame_rank = {
        "Algorithm": data_columns,
        "Average Rank": avg_ranks
    }
    df_rank = pd.DataFrame(frame_rank)
    df_friedman_test = friedman_test(data)

    #Latex table generation
    latex_output = latex_output + "\n\\begin{table}[!htp]\n"
    latex_output = latex_output + "\\centering\n"
    latex_output = latex_output + "\\caption{Average Rankings of the algorithms\n}" 
    latex_output = latex_output + "\\begin{tabular}{c|c}\n" 
    latex_output = latex_output + "Algorithm&Ranking\\\\\n\\hline";

    for i in range(len(data_columns)): 				
      latex_output = latex_output + "\n" + data_columns[i].replace("%", "\%") +"&"+ str(avg_ranks[i])+"\\\\"

    latex_output = latex_output + "\n" 
    latex_output = latex_output + "\\end{tabular}\n" 
    latex_output = latex_output + "\\end{table}"; 

    os.makedirs(output_dir, exist_ok=True)
    with open(os.path.join(output_dir, "Average_Friedman-{}.tex".format(indicator_name)), "w") as latex:
                latex.write(latex_output)
    
    return df_rank, df_friedman_test
